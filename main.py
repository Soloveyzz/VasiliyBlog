...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user:
            flash("Your email already exists in the database")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need log in or register for comment")
            return redirect(url_for("login"))
        new_comment = Comment(
            text=form.body.data,
    ...
    return render_template("post.html", post=requested_post, logged_in=current_user.is_authenticated, form=form, comments=comments, gravatar=gravatar)
